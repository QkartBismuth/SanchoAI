...
class LlamaService {
  ...
  Future<void> initialize({
    ...
    try {
      ...
      debugPrint('Model file exists, creating engine...');
      
      final backend = LlamaBackend();
      _engine = LlamaEngine(backend);
      
      _hasMmproj = mmprojPath != null && mmprojPath.isNotEmpty;
      ...
      if (_hasMmproj && mmprojPath != null) {
        await _engine!.loadMultimodalProjector(mmprojPath);
        debugPrint('Multimodal projector loaded');
      }
  ...
  Future<String> generate(String message, {List<Map<String, String>>? history, double? temperature, int? maxTokens, double? topP, int? topK, double? repeatPenalty, int? repeatLastN}) async {
    ...
    try {
      final promptTokens = _estimateTokens(message);
      _updateTokenCount(promptTokens: promptTokens, completionTokens: 0);
      
      final userContent = LlamaTextContent(message);
      ...
      await for (final chunk in _session!.create([userContent], params: params)) {
  ...
  Stream<String> continueGeneration(List<Map<String, String>>? history, {double? temperature, int? maxTokens, double? topP, int? topK, double? repeatPenalty, int? repeatLastN}) async* {
    ...
    try {
      final chatMessages = history.map((m) {
      ...
      final prompt = templateResult.prompt;
      debugPrint('Continue prompt: ${prompt.substring(prompt.length > 200 ? prompt.length - 200 : 0)}');
      ...
      debugPrint('Continue generation error: $e');
      yield 'Error: $e';
    } finally {
      _updateState(_state.copyWith(status: ModelStatus.ready));
    }
  ...
  String _buildContinuePrompt(List<Map<String, String>> history) {
  ...
  Future<String> generateWithImage({
    ...
    if (!_hasMmproj || _session == null) {
      return 'Error: Multimodal not loaded';
    }
  ...
  bool get isEngineReady => _engine?.isReady ?? false;

  bool get hasModel => _engine != null && _engine!.isReady;
